...
async def scan_software(request: ScanRequest):
    """Enhanced vulnerability scanning of uploaded software"""
    try:
        # Generate unique scan ID
        scan_id = str(uuid.uuid4())
        timestamp = datetime.now().isoformat()
        
        # Enhanced vulnerability detection based on file type
        file_extension = request.filename.split('.')[-1].lower() if '.' in request.filename else 'unknown'
        
        # Different file types have different vulnerability patterns
        vuln_probability = {
            'php': 0.8,    # High probability for PHP files
            'js': 0.6,     # Medium-high for JavaScript
            'py': 0.4,     # Medium for Python
            'java': 0.5,   # Medium for Java
            'cpp': 0.3,    # Lower for C++
            'c': 0.3,      # Lower for C
            'html': 0.7,   # High for HTML
            'xml': 0.5,    # Medium for XML
            'json': 0.2,   # Low for JSON
            'unknown': 0.3 # Default probability
        }
        
        probability = vuln_probability.get(file_extension, 0.3)
        num_vulns = random.choices([0, 1, 2, 3], weights=[1-probability, probability*0.4, probability*0.4, probability*0.2])[0]
        ...
        # Save to log file with enhanced metadata
        save_scan_log({
            "scan_id": scan_id,
            "timestamp": timestamp,
            "filename": request.filename,
            ...
            "total_vulnerabilities": len(vulnerabilities),
            "scan_duration": random.randint(5, 30),  # seconds
